...
- **1.5 前置事实（blog26 全文，2026-08-08）**：mHC = 残差流 ×4 扩宽 + pre/post-mapping + Sinkhorn-Knopp×20 双随机投影，每层两个（attention/MoE 各一），TRT 融合相邻 post/pre-mapping 且把 input RMSNorm 折进 mHC epilogue（free-normalization 模式再印证）；attention QKV/O GEMM = MXFP8，indexer Q 投影 = blockwise FP8/MXFP8，compressor 线性与 indexer weights 投影 = BF16；TRT 收敛的「最终流调度」（compressor + query-无关 indexer 半边先于共享 kv_a 投影预启动、事件同步）本质就是静态任务图 + 事件边——印证我们 megakernel 冻结图方法
- **Phase 1 计算 tile**（图不变，cp=1 退化执行下对数值）：1.1 logits+topk 融合（**FP4 MXFP4 打分**，mxf4 UMMA + 局部 top-K 候选堆，logits 不落 HBM）→ 1.1b topk K 对齐 V4（**已验收**：`top_k` 编译期参数 512/1024，堆/overflow/scratch 容量 1:1 右尺寸化（q_len=2 省 ~40KB smem），Modal 双 K 全过：values OK + 索引重合 1.0000，merge 路径真实触发 K=512→3 次/chunk、K=1024→2 次/chunk）→ 1.2 attention（**已验收**：fork flashinfer `hca_fp8.py` 完成，`mega_dsa_cp/tiles/hca.py`——砍 persistent/split-KV/SM103/reduction/host 侧，grid (2, q_len, B) 一 CTA-pair 一 (b,t)，k_tile 数从 K_valid 标量推导，CSA cmp 流 page=1 gather（entry id 列表即页表、8 个一批发射），win/cmp 双流 + 7 pipeline + 双 softmax 组全保留；**按 S-bucket 全静态编译**（gotcha #33：动态 shape marshal 是 cutlass#2794 已知 bug；Q 侧固定、KV 侧 S 标量动态由 bucket 容量池 + k_valid/win_valid 设备标量承担）；torch 在线 softmax 模拟参照下 5/5 过：C128A 变长/attn_sink、CSA gather 512、partials 路径、LSE log2 域，详见 phase1.2-attention-tile.md）→ 1.3 compressor（**已验收**：`tiles/compress.py`——CP-native partial/finalize，CSA 双流 + C128A online 状态机；step 级关闭检测（双 rank 同步推 stats 给属主，属主 = entry%cp）；Hadamard+MXFP4 量化对 DeepGEMM 参考**逐位一致**，CSA/C128A 合并 fp32 err ≤2e-6，fp8 entry 全在 ±1 ulp 内；cp=2 × 300 步 × B=8 全绿，详见 phase1.3-compressor-tile.md）→ 1.4 通信任务实现 → 1.5 GEMM tiles
