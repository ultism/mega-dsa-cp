...
33. **动态张量维度（`sym_int` / `make_fake_compact_tensor`）禁用于 launch grid 与 layout 依赖值**（NVIDIA/cutlass#2794，4.7.0 未修）：动态 shape marshal 到 launcher 时解析为 0、stride 为垃圾值。`grid.z = tensor.shape[i]` 为 0 → `cuLaunchKernelEx` 回 `CUDA_ERROR_INVALID_CONFIGURATION`（error 9，极具误导性——表象像 launch config 错，实际是 marshal 错）；垃圾 stride 还会让 C 侧运行期补全的 TMA 描述符非法 → kernel 执行后 XID 13 Illegal Instruction Parameter（`WARP_ILLEGAL_INSTR_PARAM at UTMALDG` 同类）。修法：**全部静态形状编译**（固定 shape 部署直接静态化，compile cache key 含全部维度），或显式 Int32 传参（logits.py 的 dims 模式——它因此从未踩过）。调试方法论：同签名空 stub kernel（body 立即 return）能把 launch-config 问题与 kernel 体问题干净分离；`compiled.ir_module` 可读 launcher 生成的 MLIR（grid/block/smem/attrs/参数打包全可见）；上游 hca_fp8.py 在 Modal 4.7.0 下连编译都过不了（scheduler 的 `__new_from_mlir_values__` 版本 skew）——fork 时上游代码只能当语义参照，不能直接跑。
34. **fp8-P 在线 softmax 的参考实现必须模拟 running-max 量纲**（Phase 1.2 验收实测）：kernel 的 P 按 running max 量化 fp8、再乘 exp2 校正因子；naive 参考按 final max 一次量化 → 系统性单边差异（raw fp8 V 幅度 448 时 abs 差 2.7+，且随 tile 数增长）。正确参照 = 逐 tile 模拟（`ref_hca_online`：running max + 每 tile fp8 P + corr rescale），对齐后 kernel-vs-sim 与 sim-vs-naive 同量级即验证通过。残余噪声 = ex2.approx 与 torch exp2 之差造成的 fp8 边界翻转，幅度 ∝ max|V| × ulp(P)——容差按此定（rtol/atol 2e-2 @ |V|≤448）。
65. **`shuffle_sync_bfly`/`shuffle_sync` 等默认全掩码（0xffffffff）的原语禁止出现在部分 lane 活跃的区域**（如 `if tidx < 4:` 的块内二级归约）——PTX `shfl.sync` 要求掩码内所有 lane 都执行该指令，缺 lane 即挂死（表象：首个执行该路径的 step 卡死，对端在后续事件/集合通信处超时，极具误导性）。修法：全 warp 参与 + 无效 lane 用 `sel_f32` 乘零，或改用 smem 归约。1.3 finalize 的 4-lane 二级归约中招（2026-08-08）。

36. **裸 inline-asm 的 `red.*.global` 过不了 NVVM 后端**（`red.relaxed.device.global.add.u32 [$0],$1;` 形态，NVVM backend compilation failed，无任何具体诊断）：用 DSL 内建 `cute.arch.red(ptr, val, op=, dtype=, sem=, scope=)`（events/ptx.py 范式，scope 取值仅 cluster/cta/gpu/sys）。probe：tests/probe_merge_compile.py。另：`Int32(Int64)` 构造窄化未验证（未定罪，1.4b 用 inline asm 提取字节绕开）；smem 专用 `atom.shared` 系列（atom_add_u32）不能用于 gmem 指针。
